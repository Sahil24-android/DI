...
class PaymentsAdapter(private val context: Context) : RecyclerView.Adapter<PaymentsAdapter.PaymentsViewHolder>() {

    private var paymentsList = ArrayList<EventPayment>()

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): PaymentsAdapter.PaymentsViewHolder {
    ...
    override fun onBindViewHolder(holder: PaymentsAdapter.PaymentsViewHolder, position: Int) {
        val current = paymentsList[position]
        holder.sequence.text = (position+1).toString()
        holder.paymentDate.text = current.paidDate
...
//            }
    ...
    fun updateList(newList: ArrayList<EventPayment>) {
        paymentsList.clear()
